...
import { Video, Plus, X, Trash2, Clock, CheckCircle, XCircle, AlertTriangle, Link as LinkIcon, MessageSquare } from "lucide-react"
...
export default function TacPhamPage() {
  const {
    ...
    penalty, approvedCount, totalPoints,
  } = useVideoSubmissions()
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-1">Gửi video bài dự thi của bạn</p>
        </div>
        <button
      ...
      {penalty?.penalized && (
        <div className="flex items-start gap-3 p-4 rounded-xl bg-red-400/5 border border-red-400/20">
          ...
          <div>
            <p className="text-sm font-semibold text-red-400">Bạn đã bị trừ 1 điểm!</p>
            <p className="text-xs text-zinc-400 mt-1">
              Đã {penalty.daysOverdue} ngày bạn không gửi tác phẩm. Hãy gửi bài mới để tránh bị trừ thêm điểm.
            </p>
          </div>
        </div>
      )}
      ...
      {showForm && (
        <div className="glass-strong rounded-2xl p-5 border border-white/6">
          <h2 className="text-sm font-semibold text-white mb-4">Gửi tác phẩm mới</h2>
          <form onSubmit={(e) => { e.preventDefault(); handleCreate(e); setShowForm(false) }} className="space-y-3">
            <input
              type="text"
              value={title}
            ...
            <div className="flex items-center gap-2 p-3 rounded-xl bg-amber-400/5 border border-amber-400/10">
              ...
              <p className="text-xs text-zinc-400">
                Tác phẩm sẽ được <span className="text-amber-400 font-semibold">quản trị viên duyệt</span>{" "}
                trước khi cộng <span className="text-amber-400 font-semibold">+1 điểm Truyền cảm hứng</span>
              </p>
            </div>
            <button
              type="submit"
      ...
      <div className="space-y-3">
        <h2 className="text-sm font-semibold text-white flex items-center gap-2">
          <Video size={14} className="text-primary" /> Bài đã gửi ({submissions.length})
        </h2>
        ...
        {loading ? (
          ...
          <div className="glass-strong rounded-2xl p-8 text-center">
            ...
            <p className="text-xs text-zinc-500">Chưa có tác phẩm nào. Hãy gửi bài đầu tiên!</p>
          </div>
        ) : (
          submissions.map((sub) => (
            <div key={sub.id} className="glass-strong rounded-2xl p-4 border border-white/6 group">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    ...
                    {sub.points > 0 && (
                      <span className="text-[10px] font-bold text-amber-400">+{sub.points}</span>
                    )}
                  </div>
                  {sub.adminNote && (
                    <p className="text-[10px] text-red-400/70 mt-1 italic">Phản hồi: {sub.adminNote}</p>
                  )}
                </div>
                <button
