...
export default function SettingsTab() {
  ...
  return (
    <div ref={listRef} className="space-y-8 px-2">
      {/* Flow settings */}
      <div className="mf-section rounded-2xl p-8 space-y-6" style={sectionStyle}>
        <div className="text-center">
          <div className="inline-flex p-2.5 rounded-xl mb-3" style={{ background: "color-mix(in srgb, var(--clr-primary) 10%, transparent)" }}>
            <Settings size={20} style={{ color: "var(--clr-primary)" }} />
          </div>
          <h3 className="text-base font-semibold">Cấu hình luồng hội viên</h3>
          <p className="text-xs mt-1" style={{ color: "var(--text-tertiary)" }}>Thiết lập thông tin cơ bản cho luồng đăng ký</p>
        </div>

        <div className="max-w-lg mx-auto space-y-5">
          <div>
            <label className="text-xs font-medium mb-1.5 block" style={{ color: "var(--text-secondary)" }}>Tên luồng</label>
            <input
              value={flowName}
          ...
          <div>
            <label className="text-xs font-medium mb-1.5 block" style={{ color: "var(--text-secondary)" }}>Phí hội viên (VNĐ)</label>
            <input
              type="number"
              value={flowPrice}
          ...
          <div>
            <label className="text-xs font-medium mb-1.5 block" style={{ color: "var(--text-secondary)" }}>Nội quy</label>
            <textarea
              value={flowRules}
          ...
          {message && (
            <p className="text-xs text-center font-medium" style={{ color: message.startsWith("Đã") ? "var(--color-success)" : "var(--danger)" }}>
              {message}
            </p>
          )}
      ...
      <div className="mf-section rounded-2xl p-8 space-y-6" style={sectionStyle}>
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: "var(--text-tertiary)" }}>{settings?.lessons?.length || 0} bài học</p>
          </div>
        ...
        {settings && settings.lessons.length > 0 ? (
          ...
          <p className="text-xs text-center py-8" style={{ color: "var(--text-dim)" }}>Chưa có bài học nào</p>
        )}
      </div>
      ...
      <div className="mf-section rounded-2xl p-8 space-y-6" style={sectionStyle}>
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: "var(--text-tertiary)" }}>{settings?.quizQuestions?.length || 0} câu hỏi</p>
          </div>
        ...
        {settings && settings.quizQuestions.length > 0 ? (
          ...
          <p className="text-xs text-center py-8" style={{ color: "var(--text-dim)" }}>Chưa có câu hỏi nào</p>
        )}
      </div>
      ...
      <div className="mf-section rounded-2xl p-8 space-y-6" style={sectionStyle}>
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: "var(--text-tertiary)" }}>{settings?.examSets?.length || 0} bộ đề</p>
          </div>
          <AddExamSetButton
      ...
      <div className="mf-section rounded-2xl p-8 space-y-6" style={sectionStyle}>
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: "var(--text-tertiary)" }}>{settings?.situationQuestions?.length || 0} câu hỏi</p>
          </div>
        ...
        {settings && settings.situationQuestions.length > 0 ? (
          ...
          <p className="text-xs text-center py-8" style={{ color: "var(--text-dim)" }}>Chưa có câu hỏi tình huống nào</p>
        )}
      </div>
    </div>
  )
...
export function LessonCard({ lesson, onSuccess }: { lesson: MembershipSettings["lessons"][0]; onSuccess: () => void }) {
  ...
  if (editing) {
    return (
      <div className="rounded-xl p-4 space-y-3" style={{ background: "color-mix(in srgb, var(--text-primary) 3%, transparent)" }}>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => setEditing(false)}
            className="px-4 py-2 rounded-lg text-sm cursor-pointer" style={{ color: "var(--text-tertiary)" }}>
            Hủy
          </button>
        </div>
      </div>
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-xl p-4" style={{ background: "color-mix(in srgb, var(--text-primary) 3%, transparent)" }}>
      <div className="min-w-0 flex-1">
        <p className="text-sm font-medium truncate">{lesson.title}</p>
        <div className="flex items-center gap-2 mt-0.5">
          <span className="text-[11px] px-1.5 py-0.5 rounded-md" style={{ background: "color-mix(in srgb, var(--clr-primary) 10%, transparent)", color: "var(--clr-primary)" }}>
            {lesson.type}
          </span>
          <span className="text-[11px]" style={{ color: "var(--text-dim)" }}>Thứ tự {lesson.orderIndex}</span>
        </div>
      </div>
...
function Modal({ open, onClose, children }: { open: boolean; onClose: () => void; children: React.ReactNode }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative z-10 w-full max-w-lg rounded-2xl p-6 shadow-2xl" style={{ background: "var(--surface-elevated)", border: "0.5px solid var(--border-base)" }} onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} className="absolute top-4 right-4 p-1 rounded-lg transition-colors cursor-pointer hover:bg-white/5" style={{ color: "var(--text-tertiary)" }}>
          <X size={16} />
        </button>
        {children}
      </div>
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await adminService.createLesson({
        title, description, type, orderIndex: 0, content,
      } as any)
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button onClick={() => setOpen(false)}
              className="flex-1 px-4 py-2.5 rounded-xl text-sm cursor-pointer" style={{ color: "var(--text-tertiary)", background: "color-mix(in srgb, var(--text-primary) 5%, transparent)" }}>
              Hủy
            </button>
          </div>
        </div>
...
export function QuizQuestionCard({ question, onSuccess }: { question: MembershipSettings["quizQuestions"][0]; onSuccess: () => void }) {
  ...
  const handleSave = async () => {
    try { setDeleting(true); setError(null); await adminService.updateQuizQuestion(question.id, { question: qText, options, correctAnswer }); onSuccess(); setEditing(false) }
    catch (e) { setError(e instanceof Error ? e.message : "Lỗi xảy ra"); console.error("save error", e) } finally { setDeleting(false) }
  }
  const handleDelete = async () => {
    if (!confirm("Xóa câu hỏi này?")) return
    try { setDeleting(true); setError(null); await adminService.deleteQuizQuestion(question.id); onSuccess() }
    catch (e) { setError(e instanceof Error ? e.message : "Lỗi xảy ra"); console.error("delete error", e) } finally { setDeleting(false) }
  }
      ...
      <div className="rounded-xl p-4 space-y-3" style={{ background: "color-mix(in srgb, var(--text-primary) 3%, transparent)" }}>
        ...
        {Object.entries(options).map(([key, value]) => (
          <div key={key} className="flex items-center gap-2">
            <span className="text-xs font-mono font-bold w-5 text-center" style={{ color: correctAnswer === key ? "var(--color-success)" : "var(--text-dim)" }}>{key.toUpperCase()}</span>
            ...
            <button onClick={() => removeOption(key)}
              className="p-1 rounded cursor-pointer hover:bg-[var(--glass-hover)]" style={{ color: "var(--text-dim)" }}>
              <X size={12} />
            </button>
          </div>
        ))}
...
export function AddQuizQuestionButton({ onSuccess }: { onSuccess: () => void }) {
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true)
      await adminService.createQuizQuestion({ question, options, correctAnswer } as any)
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-xl p-4" style={{ background: "color-mix(in srgb, var(--text-primary) 3%, transparent)" }}>
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs mt-0.5" style={{ color: "var(--text-dim)" }}>{situation.description}</p>
      </div>
      <div className="flex items-center gap-1 shrink-0">
        <button onClick={() => setEditing(true)}
          className="p-2 rounded-lg transition-all cursor-pointer hover:bg-[var(--glass-hover)]"
    ...
    <div className="rounded-xl p-4 space-y-3" style={{ background: "color-mix(in srgb, var(--text-primary) 3%, transparent)" }}>
      <div className="flex items-center justify-between gap-2">
        ...
        >
          ...
          <span className="text-[11px] px-2 py-0.5 rounded-full shrink-0" style={{ background: "color-mix(in srgb, var(--clr-primary) 12%, transparent)", color: "var(--clr-primary)" }}>
            {examSet.questions.length} câu
          </span>
          <span className="text-[11px] px-2 py-0.5 rounded-full shrink-0" style={{ background: "color-mix(in srgb, var(--text-primary) 8%, transparent)", color: "var(--text-tertiary)" }}>
      ...
      {editing && (
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs" style={{ color: "var(--text-dim)" }}>Điểm đậu:</span>
            <input
              type="number"
              value={passScore}
      ...
      {expanded && (
        <div className="pt-3 space-y-2 border-t" style={{ borderColor: "var(--border-base)" }}>
          <p className="text-xs font-medium" style={{ color: "var(--text-dim)" }}>
            Câu hỏi trong bộ đề:
          </p>
          {examSet.questions.length === 0 ? (
            <p className="text-xs" style={{ color: "var(--text-dim)" }}>Chưa có câu hỏi</p>
          ) : (
            examSet.questions.map((eq) => (
              <div key={eq.id} className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{ background: "color-mix(in srgb, var(--text-primary) 4%, transparent)" }}>
            ...
            <>
              {!addingQuestions ? (
                ...
                <div className="space-y-2 pt-1">
                  <p className="text-xs font-medium" style={{ color: "var(--text-dim)" }}>Chọn câu hỏi để thêm:</p>
                  <div className="max-h-48 overflow-y-auto space-y-1">
                    {availableQuestions.map((q) => (
                      <label key={q.id} className="flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer hover:bg-[var(--glass-hover)]" style={{ background: "color-mix(in srgb, var(--text-primary) 3%, transparent)" }}>
                        <input
                          type="checkbox"
                  ...
                  <div className="flex gap-2 pt-1">
                    <button
                      ...
                      style={{ background: "var(--clr-primary)", color: "#fff" }}
                    >
                      {saving ? <Loader2 size={12} className="animate-spin" /> : `Thêm (${selectedQIds.length})`}
                    </button>
                    <button
    ...
    <>
      ...
      <Modal open={open} onClose={() => setOpen(false)}>
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm" style={{ color: "var(--text-dim)" }}>Điểm đậu:</span>
          ...
          {allQuestions.length > 0 && (
            <div>
              <p className="text-xs font-medium mb-1.5" style={{ color: "var(--text-dim)" }}>Chọn câu hỏi cho bộ đề (không bắt buộc):</p>
              <div className="max-h-40 overflow-y-auto space-y-1 rounded-xl p-2" style={{ background: "color-mix(in srgb, var(--text-primary) 3%, transparent)" }}>
                {allQuestions.map((q) => (
                  <label key={q.id} className="flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer hover:bg-[var(--glass-hover)]" style={{ background: "color-mix(in srgb, var(--text-primary) 3%, transparent)" }}>
