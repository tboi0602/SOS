...
export default function ProfileHero({
  ...
  return (
    <div className="relative overflow-hidden rounded-[2.5rem] p-6 sm:p-10 group" style={{ background: "color-mix(in srgb, var(--surface-elevated) 18%, transparent)", boxShadow: "0 4px 24px color-mix(in srgb, var(--clr-primary) 10%, transparent)", border: "0.5px solid var(--border-base)" }}>
      ...
      <div className="relative flex flex-col md:flex-row items-center md:items-start gap-8">
        ...
        <div className="flex-1 text-center md:text-left space-y-3">
          <div className="space-y-1">
            ...
            <p title="Nghề nghiệp" className="text-sm font-medium flex items-center justify-center md:justify-start gap-1.5" style={{ color: "var(--text-tertiary)" }}>
              <Briefcase size={14} className="text-accent" />{" "}
              {user.job || "Chưa cập nhật"}
            </p>
          </div>

          <p className="text-xs max-w-xl italic font-light leading-relaxed" style={{ color: "var(--text-tertiary)" }}>
            `&quot;`
            {user.bio ||
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 pt-2">
            <div className="flex items-center gap-2 px-4 py-2 rounded-xl shadow-inner" style={{ background: "color-mix(in srgb, var(--text-primary) 3%, transparent)", borderColor: "var(--border-base)" }}>
              <Trophy size={14} className="text-amber-400" />
              <span className="text-xs font-medium" style={{ color: "var(--text-secondary)" }}>{rank}</span>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 rounded-xl shadow-inner" style={{ background: "color-mix(in srgb, var(--text-primary) 3%, transparent)", borderColor: "var(--border-base)" }}>
              <Star size={14} className="text-primary" />
              <span className="text-xs font-semibold" style={{ color: "var(--text-secondary)" }}>
                {score}{" "}
                <span className="text-[10px] font-normal" style={{ color: "var(--text-tertiary)" }}>Điểm</span>
              </span>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 rounded-xl shadow-inner" style={{ background: "color-mix(in srgb, var(--text-primary) 3%, transparent)", borderColor: "var(--border-base)" }}>
              ...
              <span className="text-xs font-medium" style={{ color: "var(--text-secondary)" }}>
                {referredCount}{" "}
                <span className="text-[10px] font-normal" style={{ color: "var(--text-tertiary)" }}>Đối tác</span>
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center justify-center md:justify-start gap-2.5 mt-5 pt-4 border-t" style={{ borderColor: "var(--border-base)" }}>
        {user.address && (
          <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-medium" style={{ background: "color-mix(in srgb, var(--text-primary) 3%, transparent)", color: "var(--text-tertiary)" }}>
        ...
        {[
          ...
          s.url ? (
            <a
              ...
              style={{ background: "color-mix(in srgb, var(--clr-accent) 8%, transparent)", color: "var(--clr-accent)", border: "0.5px solid color-mix(in srgb, var(--clr-accent) 15%, transparent)" }}
            >
