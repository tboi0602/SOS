...
export default function ReferralField({
  label, id, value, onChange, placeholder, className, status, name,
}: {
  ...
  const borderColor =
    status === "valid" ? "border-emerald-500/50"
      : status === "invalid" ? "border-danger/50"
        : "border-white/10"
    ...
    <div className="space-y-1.5">
      ...
      <div className="relative">
        ...
        <div className="absolute right-3 top-1/2 -translate-y-1/2">
          ...
          {status === "valid" && <CheckCircle2 size={16} className="text-emerald-400" />}
          {status === "invalid" && <AlertCircle size={16} className="text-danger" />}
        </div>
      </div>
      {status === "valid" && name && (
